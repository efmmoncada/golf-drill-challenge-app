...
function createPlayer() {
    ...
    const email = faker.internet.email(name.split(' ')[0], name.split(' ')[1], "oregonstate.edu");
    return {
        name,
        email,
        completedDrills: faker.helpers.arrayElements(completedDrills, Math.floor(Math.random() * 10) % 4 + 1),
        assignedDrills: faker.helpers.arrayElements([1, 2, 3, 4, 5, 6, 7, 8, 9], Math.floor(Math.random() * 10) % 6 + 1)
    };
}
...
function createDrill() {
    ...
    const media = new Array(Math.floor(Math.random() * 10) % 4 + 1).fill(0).map(() => faker.image.sports());
    return {
        id,
